...
function dockCorner(display, dock, window, margin = 0) {
...
function fitDockLayout(corner, character, bubble = null) {
  ...
  const scale = Math.min(1,
    ...
    (area.height - (horizontal ? 0 : gap) - rounding) / totalHeight);
  ...
  if (!bubble) return { ...corner, scale, character: charBounds, bubble: null };
  ...
  return { ...corner, scale, character: charBounds, bubble: { ...bubbleBounds, tailSide, tailOffset: Math.max(12, Math.min(extent - 24, offset)) } };
}
