...
- **Rendering engine (3D)**: `src/engine/vibemon-engine-3d.js` builds a procedural digital vibe monster (primitives + flat materials, no models/textures) — a squishy horned egg body with big glinted eyes, cheek blushes, nub arms, a spade tail, and a "vibe flame" (unlit MeshBasicMaterial) burning above its head — with THREE.Group pivots for body/arms/tail chain/flame. Each state sets a `vibe` energy level driving the flame's size/flicker and tail sway (sleep 0.25× → alert 2.6×), and state motion is creature behavior only (gazing around, waving, tail-tapping, scanning, orbit-drifting, busy bouncing, heartbeat pulsing, coiling, popping up, pirouetting, nodding off, sinking, shivering, darting glances — no human-tool mimicry; `STATE_ANIMATIONS`/`MOVES` in monster-states.js). Each of the 10 states maps to a target pose + named procedural moves (`monster-states.js`: `STATE_ANIMATIONS`, `MOVES`, `CHARACTER_THEMES`); poses blend smoothly on state change, the loop is capped at ~30fps, and the WebGL canvas fills the 172x160 window with the floating animation done in-scene. `tests/monster-states.test.js`/`tests/engine-3d.test.js` guard that every registry state has an animation and every move/joint referenced is implemented.
