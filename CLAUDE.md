...
- **Tray usage display**: `usage-cache-reader.cjs` reads the same `~/.vibemon/cache/usage.json` directly (independent of `usage5h`/`usageWeek` on any tracked project's status) so `tray-manager.cjs` can show Claude and Codex plan usage (5h + weekly %, with time-to-reset) at the top of the tray menu, account-level regardless of which project is focused; buckets with stale or missing data are omitted rather than shown as a placeholder
