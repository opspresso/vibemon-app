...
- **Rendering engine**: `src/engine/vibemon-engine-3d.js` (three.js articulated pet on a transparent WebGL canvas, default) and `src/engine/vibemon-engine.js` (2D pixel-art engine, vendored from vibemon-static) + `src/engine/monster-states.js` (pure state/theme definitions, no three import) + `src/vendor/three.module.min.js`/`three.core.min.js` (vendored three.js — CSP `script-src 'self'` requires local files). `src/assets/characters/*.png` are the 2D sprites (remote-first from static.vibemon.io, bundled as offline fallback) and the tray icon source
...
- **Render mode**: persisted `renderMode` setting (`'3d'` default, or `'2d'`) selects which engine the character window boots — `src/engine/vibemon-engine.js` (2D pixel-art sprite) or `src/engine/vibemon-engine-3d.js` (3D pet); renderer.js dynamic-imports by mode (`electronAPI.getRenderMode()`), and `setRenderMode()` reloads the open window and replays its state. Switch via tray menu (**Render Mode**) or Settings → VibeMon.
- **Rendering engine (3D)**: `src/engine/vibemon-engine-3d.js` builds a procedural jointed pet (primitives + flat toon materials, no models/textures) with kawaii proportions — one blob silhouette (huge head overlapping a squat body), big glossy wide-spaced eyes set low on the face, small rounded ears, cheek blushes, stubby limbs, chubby tail — with THREE.Group pivots for head/shoulders/elbows/legs/tail. Each of the 10 states maps to a target pose + named procedural moves (`monster-states.js`: `STATE_ANIMATIONS`, `MOVES`, `CHARACTER_THEMES`); poses blend smoothly on state change, the loop is capped at ~30fps, and the WebGL canvas fills the 172x160 window with the floating animation done in-scene. `tests/monster-states.test.js`/`tests/engine-3d.test.js` guard that every registry state has an animation and every move/joint referenced is implemented.
...
- **Interaction expression + manual drag**: From the moment the character is pressed (pointerdown) until release, the renderer temporarily overrides the engine state with `done` (happy eyes) and restores the last reported state afterwards. The display area is deliberately *not* an app-region drag surface (that would swallow pointerdown), so dragging is manual: pointerdown anchors the drag (`window-drag-start` IPC → `beginUserDrag()`), pointermove asks the main process to move the window along with the cursor (`window-drag-move` → `moveUserDrag()`, reading the cursor via `screen.getCursorScreenPoint()`), and the resulting `move` events feed the usual snap/persist debounce. Clicks that dragged the window don't trigger click-to-focus
